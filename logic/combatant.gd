...
func add_card_to_graveyard(
	...
	if p_from_zone == "lane" and p_instance_id_from_origin_zone != -1: # p_instance_id_from_origin_zone is the summon's field ID
		# We need the lane_index. This method doesn't know it.
		# This implies that when a summon dies, the "card_moved" (lane->graveyard)
		# should perhaps be generated by SummonInstance.die() itself, which *does* know the lane,
		# rather than by add_card_to_graveyard.
		# OR, add_card_to_graveyard needs more context if from_zone is "lane".

		# For now, let's keep it simpler: the crucial part is to_details.instance_id.
		# The "creature_defeated" event already tells us which lane the summon died in.
		# The "instance_id" of this card_moved event (event_main_instance_id) also points to the summon that was in the lane.
		# So, explicit from_details.lane might be redundant if the replay can correlate.
		pass


	# If a specific effect caused this move to graveyard (e.g., a spell saying "target creature dies and goes to grave")
	# that source info would need to be passed into add_card_to_graveyard.
	# Currently, if it's from SummonInstance.die(), the "source" of the card_moved is the death itself.
