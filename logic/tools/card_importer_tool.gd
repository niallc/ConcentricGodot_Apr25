...
func _run():
	...
	for card_entry in json_data:
		...
		var resource_path = INSTANCE_DIR + card_id.to_lower() + ".tres"
		var effect_script_path = EFFECT_SCRIPT_DIR + card_id.to_lower() + "_effect.gd"
		...
		if ResourceLoader.exists(resource_path):
			...
			# Check if the loaded resource's script inherits from the expected base script
			...
			if loaded_script == null or not loaded_script.is_valid():
				...
				error_count += 1
				continue
			# BaseEffectScript holds the preloaded base .gd script (SpellCardResource or SummonCardResource)
...
func validate_card_entry(entry: Dictionary) -> bool:
	if not entry.has("id") or not entry["id"] is String or entry["id"].is_empty():
		printerr("Missing or invalid 'id'.")
		return false
...
func ensure_dir_exists(path: String):
	# DirAccess.make_dir_recursive_absolute(path) # Godot 4.3+ ? Check docs
	# Manual check/create for broader compatibility:
...
func create_placeholder_effect_script(script_path: String, base_script_path: String):
	var base_class_name = base_script_path.get_file().replace(".gd", "") # e.g., "spell_card" -> Needs class_name defined!
	# Or get class_name directly if possible? Safer to use path for extends.
